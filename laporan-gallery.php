...
$settings['width'] = 166;				// Default width
//$settings['height'] = 166;				// Default height
...
//$settings['thumb_width'] = 166;
//$settings['thumb_height'] = 166;



$dimensions = woo_laporan_image_dimensions( $woo_options['woo_layout'], $woo_options['woo_layout_width'] );
...
$rwmb_images = rwmb_meta( 'KOKI_foto', $args = array('type'=>'image', 'size'=>'thumbnail'), $post->ID );
$first_rwmb_images = array_shift($rwmb_images);
//print_r($rwmb_images);
/*-----------------------------------------------------------------------------------*/
/* Generate the HTML to be outputted, if applicable. */
/*-----------------------------------------------------------------------------------*/

if ( ! empty( $rwmb_images ) ) {
	...
	foreach ( $rwmb_images as $k => $img ) {
		...
		$before = '<div class="' . $slide_container_class . '"' . $style . '><a href="'. $img['full_url'] .'" rel="' . $rel . '" class="thickbox"' . $title . '>'; 
		$after = '</a>' . "\n" . $caption . '</div><!--/.slide-->' . "\n";
		...
		//$rwmb_img = '<img src="'..'" alt="" class="woo-image"  width="166"  />';
	}
...
if ( $html != '' ) {
	echo '<div id="post-gallery" class="laporan-img">' . woo_image( 'noheight=true&return=true&key=laporan-image&width=520&class=laporan-img' ) . '</div><!--/#post-gallery .laporan-img-->' . "\n";
