<?php
/**
 * Template Name: Form Laporan Warga PetaBDG.com
 * 
 * The Form Template
 *
 * @author Wisya Yugaswara J
 */
function tambah_laporan_redirect($url)
{
   header('Location: ' . $url);
   die();
}
function tambah_laporan_enqueue($hook) {
...
add_action( 'wp_enqueue_scripts', 'tambah_laporan_enqueue' );
...
// function do_insert() {	
if( 'POST' == $_SERVER['REQUEST_METHOD'] || wp_verify_nonce( $_POST['laporan-baru'] ) ) { // Check what the post type is here instead
    ...
    if(isset($_POST['bukalah']) && ($_POST['bukalah']=='022') &&
       ...
       isset($_POST['pengirim'])) {
        ...
        $post = array(
        	...
        	'tax_input'	=> array('kategori-laporan'=>$cat), 
        	// 'tags_input'	=> $tags,
        	'post_status'	=> 'draft', // Choose: publish, preview, future, etc.
        	'post_type'	=> 'laporan' // Set the post type based on the IF is post_type X
        );

        $insert_laporan = wp_insert_post($post); // = post_ID

        add_post_meta( $insert_laporan, 'KOKI_map', $_POST['map'], false );
        add_post_meta( $insert_laporan, 'KOKI_alamat', $_POST['alamat'], false );
        add_post_meta( $insert_laporan, 'KOKI_website', $_POST['website'], false );
        add_post_meta( $insert_laporan, 'KOKI_telepon', $_POST['telepon'], false );
        add_post_meta( $insert_laporan, 'KOKI_pengirim', $_POST['pengirim'], false );
        add_post_meta( $insert_laporan, 'KOKI_telepon_pengirim', $_POST['telepon_pengirim'], false );
        add_post_meta( $insert_laporan, 'KOKI_email_pengirim', $_POST['email'], false );
        add_post_meta( $insert_laporan, 'KOKI_twitter_pengirim', $_POST['twitter'], false );
        if(isset($_POST['kategori_lainnya']) && $_POST['kategori_lainnya']!='')
        	add_post_meta( $insert_laporan, 'KOKI_kategori_lainnya', $_POST['kategori_lainnya'], false );
            ...
            foreach ($files['name'] as $key => $value) {             
                if ($files['name'][$key]) {
                    ...
                    foreach ($_FILES as $file => $array) {                
                        $newupload = koki_handle_attachment($file,$insert_laporan, true);
                        // print_r($newupload);
                        // echo "<hr/>";
                    }
                }
    	...
    	$preview_posts[$insert_laporan] = true;
    	update_option('simple_preview', $preview_posts); 
    	tambah_laporan_redirect('http://www.petabdg.com/lapor/?nuhun=' . $insert_laporan);
    }
}
...
jQuery(document).ready(function(){
    // jQuery("#tambah_laporan").validate({
    //     rules: {
    //         title: "required",
    //         title: "required",
    //         map: "required",
    ...
    <div id="content" class="col-full">
    	...
    	<div id="main-sidebar-container">
			...
			if(isset($_GET['nuhun']) AND $_GET['nuhun']!='') {
                echo do_shortcode('[box]<p style="text-align:center">Terimakasih atas laporan anda.<br/>
                Akan segera kami teruskan kepada pihak terkait.<br/>
                Preview laporan bisa dicek di link ini:<br/>
                ( <a href="'.get_permalink($_GET['nuhun']).'&preview=true" target="_blank" style="font-weight:bold">'.get_the_title($_GET['nuhun']).'</a> )<br/>
                Bila anda ingin melaporkan hal lainnya, silakan isi form ini kembali.<br/>
                (^_^)</p>[/box]'); 
			}
			?>
			...
			<form action="" id="tambah_laporan" method="post" enctype="multipart/form-data" name="front_end_upload" >
			 ...
			 <ol class="forms">
                ...
                <li><label>Jenis Laporan</label><?php wp_dropdown_categories( 'tab_index=10&taxonomy=kategori-laporan&hide_empty=0&hide_if_empty=0&orderby=SLUG&order=ASC&class=cat' ); ?> 
                ...
                <li class="textarea"><label for="description">Informasi Selengkapnya</label>
                    <textarea name="description" id="description" rows="10" cols="20" class="requiredField" required></textarea>
                </li>
            ...
            <div class="container">
				...
				<?php wp_nonce_field( 'laporan-baru' ); ?>
            </ol>
			</form>
            </div>
            </section><!-- /#main -->
